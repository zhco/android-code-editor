...
class FileTreeFragment(private val onFileSelected: (File) -> Unit) : Fragment() {
        ...
        return LinearLayout(ctx).apply {
            ...
            fun addBtn(id: Int): Button {
                return Button(ctx).apply {
                    ...
                    setOnClickListener {
                        when(id) {
                            1 -> currentDir.parentFile?.let { currentDir = it; refreshList() }
                            2 -> showNewFileDialog(false)
                            3 -> showNewFileDialog(true)
                            4 -> showImportDialog()
                        }
                    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        currentDir = requireContext().getExternalFilesDir(null) ?: Environment.getExternalStorageDirectory()
